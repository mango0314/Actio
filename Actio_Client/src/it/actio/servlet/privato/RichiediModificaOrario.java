 ...
 */
public class RichiediModificaOrario extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ActivityServiceStub.GetOrari req = new ActivityServiceStub.GetOrari();
		req.setIdCorso(idCorso);
		
		ActivityServiceStub.GetOrariResponse resp = stub.getOrari(req);
		OrarioCorsoDTO[] orari = resp.get_return();
		
		request.setAttribute("orari", orari);
		request.setAttribute("idCorso", idCorso);
		request.getRequestDispatcher( "/WEB-INF/privato/richiediModificaOrari.jsp").forward(request, response);
