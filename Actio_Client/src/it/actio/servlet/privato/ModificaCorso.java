...
public class ModificaCorso extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean modificato = false; 
        		...
        		String corso_idParam = request.getParameter("corso_id");
                String nomeCorso = request.getParameter("nomeCorso");
                String descrizione = request.getParameter("descrizione");
                String capienzaStr = request.getParameter("capienza");
                String fotoAttuale = request.getParameter("foto_attuale");
                                      
                
                boolean datiMancanti = corso_idParam == null || corso_idParam.trim().isEmpty()||
                		nomeCorso == null || nomeCorso.trim().isEmpty() ||
                ...
                int corsoId;
                try {
                    corsoId = Integer.valueOf(corso_idParam.trim());
                } catch (NumberFormatException e) {
                    response.sendRedirect(request.getContextPath() + "/Index_privato?errore=CORSO_NON_CARICATO_CORRETTAMENTE");
                    return;
                }
             ...
             // --- 3. GESTIONE FOTO ---
                String pathFotoDefinitivo = fotoAttuale; // Default: mantieni vecchia

                // Verifica se c'è un nuovo upload
                Part filePart = request.getPart("fotoCorso");
                if (filePart != null && filePart.getSize() > 0) {
                    // C'è un nuovo file: validalo e salvalo
                    String nuovoPath = validaESalvaImmagine(request, response, "fotoCorso", UPLOAD_DIR);
                    if (nuovoPath == null) return; // Errore redirect già gestito
                    
                    pathFotoDefinitivo = nuovoPath;

                    // Cancella la vecchia foto se esisteva e diversa
                    if (fotoAttuale != null && !fotoAttuale.isEmpty()) {
                        java.io.File vecchiaFoto = new java.io.File(fotoAttuale);
                ...
                corso.setFoto(pathFotoDefinitivo);
                
                ActivityServiceStub.ModificaCorso req2 = new ActivityServiceStub.ModificaCorso();
                req2.setCorso(corso);
                req2.setIdAttivita(idAttivita);
                
                ActivityServiceStub.ModificaCorsoResponse resp2 = stub.modificaCorso(req2);
            ...
            if (modificato) {
                response.sendRedirect(request.getContextPath() + "/privato/Index_privato?success=MODIFICATO");
            } else {
                response.sendRedirect(request.getContextPath() + "/privato/Index_privato?errore=MODIFICA_FALLITA");
            }
            return;
